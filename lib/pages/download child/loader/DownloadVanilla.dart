...
class _DownloadVanillaPageState extends State<DownloadVanillaPage> {
  ...
  Future<void> _DownloadLibraries({int concurrentDownloads = 20}) async {
    ...
    final _SelectedGamePath = prefs.getString('SelectedPath') ?? '';
    final _GamePath = prefs.getString('Path_$_SelectedGamePath') ?? '';
    ...
    if (_isRetrying && _failedLibraries.isNotEmpty) {
      ...
      for (int i = 0; i < librariesURL.length; i++) {
        ...
        final fullPath = '$_GamePath${Platform.pathSeparator}libraries${Platform.pathSeparator}$relativePath';
  ...
  Future<void> _DownloadAssets({int concurrentDownloads = 30}) async {
    ...
    if (_isRetrying && _failedAssets.isNotEmpty) {
      ...
      for (int i = 0; i < _assetHash.length; i++) {
        ...
        final AssetDir = '$_GamePath${Platform.pathSeparator}assets${Platform.pathSeparator}objects${Platform.pathSeparator}$hashPrefix';
...
void _startDownload() async {
  ...
  final _SelectedGamePath = prefs.getString('SelectedPath') ?? '';
  final _GamePath = prefs.getString('Path_$_SelectedGamePath') ?? '';
  final _VersionPath = '$_GamePath${Platform.pathSeparator}versions${Platform.pathSeparator}${widget.name}';
  final _GameJsonURL = replaceWithMirror(widget.url);
  // 下载版本json
  try {
    await DownloadFile('$_VersionPath${Platform.pathSeparator}${widget.name}.json', _GameJsonURL);
    setState(() {
      _DownloadJson = true;
    });
    // 解析游戏Json
    await parseGameJson('$_VersionPath${Platform.pathSeparator}${widget.name}.json');
    // 下载资产索引文件
    if (assetIndexURL != null) {
      final assetIndexDir = '$_GamePath${Platform.pathSeparator}assets${Platform.pathSeparator}indexes';
      final assetIndexPath = '$assetIndexDir${Platform.pathSeparator}$assetIndexId.json';
      // 下载资产索引
      await DownloadFile('$_GamePath${Platform.pathSeparator}assets${Platform.pathSeparator}indexes${Platform.pathSeparator}$assetIndexId.json', assetIndexURL!);
      setState(() {
      ...
      // 下载客户端
      await DownloadFile('$_VersionPath${Platform.pathSeparator}${widget.name}.jar', clientURL);
      setState(() {
